...
// GoogleAuth provides a mock function with given fields: ctx, state, userAuth
func (_m *UseCase) GoogleAuth(ctx context.Context, state string, userAuth *oauth.GoogleUserResult) (*model.GoogleAuthToken, error) {
	ret := _m.Called(ctx, state, userAuth)

	var r0 *model.GoogleAuthToken
	if rf, ok := ret.Get(0).(func(context.Context, string, *oauth.GoogleUserResult) *model.GoogleAuthToken); ok {
		r0 = rf(ctx, state, userAuth)
	} else {
		if ret.Get(0) != nil {
	...
	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context, string, *oauth.GoogleUserResult) error); ok {
		r1 = rf(ctx, state, userAuth)
	} else {
		r1 = ret.Error(1)
	}
